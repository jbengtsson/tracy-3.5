...
set title "{/Symbol \266b/\266d} [m]";
set xlabel "s [m]"; set ylabel "";
set y2range [-2.0:20];
plot file_name using 3:4 axis x1y2 notitle with fsteps lt 1 lw 1 \
     lc rgb "black", \
     file_name using 3:5 title "{/Symbol \266b}_x/{/Symbol \266d}" \
     with lines ls 1, \
     file_name using 3:6 title "{/Symbol \266b}_y/{/Symbol \266d}" \
     with lines ls 3;
if (!ps) pause mouse "click on graph to cont.\n";

if (ps) set output "plt_nl_optics_2.".(ext);
set title "{/Symbol \266h}_x/{/Symbol \266d}";
set xlabel "s [m]"; set ylabel "";
set y2range [-2.0:20];
plot file_name using 3:4 axis x1y2 notitle with fsteps lt 1 lw 1 \
     lc rgb "black", \
