...
void prtmfile(const char mfile_dat[])
{
  ...
  for (i = 0; i <= globval.Cell_nLoc; i++) {
	...
	fprintf(mfile, " %23.16e %23.16e %23.16e %23.16e\n",
		...
		Lattice.Cell[i].Elem.M->PdTsys
		+Lattice.Cell[i].Elem.M->PdTrms*Lattice.Cell[i].Elem.M->PdTrnd);
	...
	prtHOM(mfile, Lattice.Cell[i].Elem.M->n_design, Lattice.Cell[i].Elem.M->PB,
	       Lattice.Cell[i].Elem.M->Porder);
      } else {
	prtName(mfile, i, thinkick_, Lattice.Cell[i].Elem.M->Pmethod,
	...
	prtHOM(mfile, Lattice.Cell[i].Elem.M->n_design, Lattice.Cell[i].Elem.M->PB,
      ...
      break;
    case Wigl:
      prtName(mfile, i, wiggler_, Lattice.Cell[i].Elem.W->Pmethod, Lattice.Cell[i].Elem.W->PN);
    ...
    case Cavity:
      ...
      fprintf(mfile, " %23.16e %23.16e %d %23.16e %23.16e\n",
	      Lattice.Cell[i].Elem.C->Pvolt/(1e9*globval.Energy),
	      2.0*M_PI*Lattice.Cell[i].Elem.C->Pfreq/c0, Lattice.Cell[i].Elem.C->Ph,
    ...
    case marker:
      ...
      break;
    case Insertion:
      prtName(mfile, i, kick_map_, Lattice.Cell[i].Elem.ID->Pmethod,
	      Lattice.Cell[i].Elem.ID->PN);
