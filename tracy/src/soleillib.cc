...
/* void SetErr(void)
   ...
   Purpose:
   Set error
   Definir une distribution aleatoire de quadripoles tournes associee a
   chaque quadripole de la machine
...
void SetErr(void)
{
  ...
  for (i = 1L; i <= Lattice.param.Cell_nLoc; i++) {
	...
	/* random error every 2 elements (quad split into 2) */
        pair++;
        M->Bpar[HOMmax-2L] = -M->Bpar[HOMmax+2L]*sin(2.0*theta);
...
/* void fmap(long Nbx, long Nbz, long Nbtour, double xmax, double zmax,
   ...
   Comments:
   15/10/03 run for the diffusion: nasty patch for retrieving the closed 
   orbit
...
void Multipole(void)
{
  ...
  if (!trace)
    printf("Elements: ndip=%d nquad=%d  nsext=%d nhcorr=%d nvcorr=%d nqcorr=%d"
	   "\n",
			...
			long nstepp, double em_min, double em_max, long nstepm)
{
  ...
  for (i = 1L; i <= nstepp; i++){ // loop over energy
    ...
    if (deb > 1L){
      // track from 1 to deb-1L element
      Lattice.Cell_Pass(1L, deb - 1L, x0, lastpos);
      j = deb -1L;
