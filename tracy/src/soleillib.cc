...
void SetErr(LatticeType &lat)
{
  ...
  for (i = 1L; i <= lat.conf.Cell_nLoc; i++)
    {
      lat.getelem(i, &Cell);
      if (Cell.Pkind == 2L)
	{
	  ...
	  if (M->Porder == 2L && Cell.dT[1] == 0)
	    {
	      if ((pair%2)==0) theta = fac*normranf(); /* random error every 2
							  elements (quad split into
							  2) */
	      pair++;
	      M->PBpar[HOMmax-2L] = -M->PBpar[HOMmax+2L]*sin(2.0*theta);
	      M->PBpar[HOMmax+2L] =  M->PBpar[HOMmax+2L]*cos(2.0*theta);
	      if (trace)
		printf("%6s % .5e % .5e % .5e\n",Cell.PName,
	      ...
	      FILE *outf1, double Tx[][NTURN])
{
  ...
  lat.Cell_Pass(pos -1, lat.conf.Cell_nLoc, x1, lastpos);
	  ...
	  double zmax, double energy)
{
  ...
  if (fabs(xmax) > 0.0){
    ...
    for (i = 0; i <= Nbx; i++) {
      ...
      Trac_Simple(lat, x, xp, z, zp, energy, 0.0, Nbtour, Tab, &stable);
      if (stable) {
	  ...
	  double zmax, double energy, bool diffusion, bool matlab)
{
  ...
  for (i = -Nbx; i <= Nbx; i++) {
    ...
    for (j = -Nbz; j<= Nbz; j++) {
      ...
      // tracking around closed orbit
      Trac_Simple(lat, x, xp, z, zp, energy, ctau, nturn, Tab, &status);
	    ...
	    double emax, double z, bool diffusion, bool matlab)
{
  ...
  if (diffusion && lat.conf.Cavity_on == false) nturn = 2*Nbtour;
  ...
  for (i = 0; i <= Nbe; i++) {
	...
	x  = x0 + sgn(j)*sqrt((double)abs(j))*xstep;
      ...
      Trac_Simple(lat, x, xp, z, zp, dp, ctau, nturn, Tab, &status);
      if (status) {
	Get_NAFF(NTERM2, Nbtour, Tab, fx, fz, nb_freq);
	Get_freq(fx, fz, &nux1, &nuz1);  // gets nux and nuz
	if (diffusion) { // diffusion
...
void NuDp(LatticeType &lat, long Nb, long Nbtour, double emax)
{
  ...
  for (i = 0L; i < Nb; i++) {
    ...
    fprintf(outf, "%14.6e %14.6e %14.6e %14.6e %14.6e %14.6e %14.6e\n",
            dp, nux1, nuz1, lat.conf.CODvect[0], lat.conf.CODvect[1],
            lat.conf.CODvect[2], lat.conf.CODvect[3]);
    fprintf(stdout, "%14.6e %14.6e %14.6e %14.6e %14.6e %14.6e %14.6e\n",
            dp, nux1,nuz1, lat.conf.CODvect[0], lat.conf.CODvect[1],
            lat.conf.CODvect[2], lat.conf.CODvect[3]);
  }
	       ...
	       double pz0, double delta0, double ctau0, long Nbtour)
{
  ...
  fprintf(stdout, "      x            px           z           pz        delta"
	  "       ctau\n");
  fprintf(stdout,"% 12.8f % 12.8f % 12.8f % 12.8f % 12.8f % 12.8f\n",
          lat.conf.CODvect[0], lat.conf.CODvect[1], lat.conf.CODvect[2],
          lat.conf.CODvect[3], lat.conf.CODvect[4], lat.conf.CODvect[5]);
  lastpos = pos;
  lat.conf.CODvect = xsynch;
  //  xsynch[0] = lat.conf.CODvect[0];
  //  xsynch[1] = lat.conf.CODvect[1];
  //  xsynch[2] = lat.conf.CODvect[2];
  ...
  for (j = 0; j < ne; j++){
    for (i = 0; i < nx; i++){
      ...
      Trac(lat, x, px, z, pz, delta, ctau, Nbtour,pos, lastn, lastpos, outf);
      fprintf(outf,"\n");
    }
  }
  fclose(outf);
...
****************************************************************************/
void PhasePortrait(LatticeType &lat, double x0, double px0,double z0,
		   double pz0, double delta0, double ctau0, double end, long Nb,
		   long Nbtour, int num)
{
  double Tab[6][NTURN];
  FILE *outf;
  ...
  for (j = 0; j <= Nb; j++){
    ...
    Trac_Simple(lat, x, px, z, pz, delta, ctau, Nbtour, Tab, &status);
    for (i = 0; i < Nbtour; i++) {
      fprintf(outf,"% .5e % .5e % .5e % .5e % .5e % .5e\n",
	      Tab[0][i], Tab[1][i], Tab[2][i], Tab[3][i], Tab[4][i], Tab[5][i]);
    }
	       ...
	       double dp, double nturn)
{
  ...
  printf("xcod=%.5e mm zcod=% .5e mm \n",
	 lat.conf.CODvect[0]*1e3, lat.conf.CODvect[2]*1e3);
...
void Multipole(LatticeType &lat)
{
	  ...
	  if (fabs(M->Pirho) > 0.0)
	    {
	      dlist[ndip] = i;
	      ndip++;
	      if (trace) printf("%s % f\n",Cell.PName, M->PB[0 + HOMmax]);
	    }
	  else if (fabs(M->PBpar[2L + HOMmax]) > 0.0)
	    {
	      qlist[nquad] = i;
	      nquad++;
	      if (trace) printf("%s % f\n",Cell.PName, M->PBpar[2L + HOMmax]);
	    }
	  else if (fabs(M->PBpar[3L + HOMmax]) > 0.0)
	    {
	      slist[nsext] = i;
	      nsext++;
	      if (trace) printf("%s % f\n",Cell.PName, M->PBpar[3L + HOMmax]);
	    }
	  else if ( Cell.PName[0] == 'c' && Cell.PName[1] == 'h')
	    {
	      hcorrlist[nhcorr] = i;
	      nhcorr++;
	      if (trace) printf("%s \n",Cell.PName);
	    }
	  else if ( Cell.PName[0] == 'c' && Cell.PName[1] == 'v')
	    {
	      vcorrlist[nvcorr] = i;
	      nvcorr++;
	      if (trace) printf("%s \n",Cell.PName);
	    }
	  else if ( Cell.PName[0] == 'q' && Cell.PName[1] == 't')
	    {
	      qcorrlist[nqcorr] = i;
	      nqcorr++;
	      if (trace) printf("%s \n",Cell.PName);
  ...
  for (i = 0; i < ndip; i++)
    {
      ...
      /* gradient error */
      mKL = dBoB2*theta*x0i;
      SetKLpar(lat, Cell.Fnum, Cell.Knum, mOrder=2L, mKL);

      if (trace)
	printf("num= %4d name = %s Fnum = %3d, Knum=%3d theta=% e mKl=% e\n",i,
	       Cell.PName,Cell.Fnum, Cell.Knum, theta, mKL);

      /* sextupole error */
      mKL = dBoB3*theta*x02i;
      SetKLpar(lat, Cell.Fnum, Cell.Knum, mOrder=3L, mKL);

      /* octupole error */
      mKL = dBoB4*theta*x03i;
      SetKLpar(lat, Cell.Fnum, Cell.Knum, mOrder=4L, mKL);

      /* decapole error */
      mKL = dBoB5*theta*x04i;
      SetKLpar(lat, Cell.Fnum, Cell.Knum, mOrder=5L, mKL);

      /* 12-pole error */
      mKL = dBoB6*theta*x05i;
      SetKLpar(lat, Cell.Fnum, Cell.Knum, mOrder=6L, mKL);

      /* 14-pole error */
      mKL = dBoB7*theta*x06i;
      SetKLpar(lat, Cell.Fnum, Cell.Knum, mOrder=7L, mKL);
    }
  ...
  for (i = 0; i < nquad; i++)
    {
      ...
      b2 = Cell.PL*GetKpar(lat, Cell.Fnum, Cell.Knum, 2L);

      /* 12-pole multipole error */
      if ((strncmp(Cell.PName,"qp2",3)==0) || (strncmp(Cell.PName,"qp7",3)==0))
	mKL= b2*dBoB6L*x04i;
      else
	mKL= b2*dBoB6C*x04i;
      SetKLpar(lat, Cell.Fnum, Cell.Knum, mOrder=6L, mKL);
	...
	printf("num= %4d name = %s Fnum = %3d, Knum=%3d b2=% e mKl=% e\n",i,
	       Cell.PName,Cell.Fnum, Cell.Knum, b2, mKL);
	...
	mKL= b2*dBoB10C*x08i;
      SetKLpar(lat, Cell.Fnum, Cell.Knum, mOrder=10L, mKL);
      if (trace)
	printf("num= %4d name = %s Fnum = %3d, Knum=%3d b2=% e mKl=% e\n",i,
	       Cell.PName,Cell.Fnum, Cell.Knum, b2, mKL);
	...
	mKL= b2*dBoB14C*x012i;
      SetKLpar(lat, Cell.Fnum, Cell.Knum, mOrder=14L, mKL);
      if (trace)
	printf("num= %4d name = %s Fnum = %3d, Knum=%3d b2=% e mKl=% e\n",i,
	       Cell.PName,Cell.Fnum, Cell.Knum, b2, mKL);
  ...
  for (i = 0; i < nsext; i++)
    {
      ...
      b3 = GetKpar(lat, Cell.Fnum, Cell.Knum, 3L);

      /* 18-pole multipole error */
      mKL= b3*dBoB9*x06i;
      SetKLpar(lat, Cell.Fnum, Cell.Knum, mOrder=9L, mKL);
      if (trace)
	printf("num= %4d name = %s Fnum = %3d, Knum=%3d b3=% e mKl=% e\n",i,
	       Cell.PName,Cell.Fnum, Cell.Knum, b3, mKL);

      /* 30-pole multipole error */
      mKL= b3*dBoB15*x012i;
      SetKLpar(lat, Cell.Fnum, Cell.Knum, mOrder=15L, mKL);
      if (trace)
	printf("num= %4d name = %s Fnum = %3d, Knum=%3d b3=% e mKl=% e\n",i,
	       Cell.PName,Cell.Fnum, Cell.Knum, b3, mKL);
      ...
      mKL= b3*dBoB21*x018i;
      SetKLpar(lat, Cell.Fnum, Cell.Knum, mOrder=21L, mKL);
      if (trace)
	printf("num= %4d name = %s Fnum = %3d, Knum=%3d b3=% e mKl=% e\n",i,
	       Cell.PName,Cell.Fnum, Cell.Knum, b3, mKL);
      ...
      mKL= b3*dBoB27*x024i;
      SetKLpar(lat, Cell.Fnum, Cell.Knum, mOrder=27L, mKL);
      if (trace)
	printf("num= %4d name = %s Fnum = %3d, Knum=%3d b3=% e mKl=% e\n",i,
	       Cell.PName,Cell.Fnum, Cell.Knum, b3, mKL);
    }
  ...
  for (i = 0; i < nhcorr; i++)
	...
	printf("num= %4d name = %s Fnum = %3d, Knum=%3d BL/brho=% e mKl=% e\n",
	       i, Cell.PName,Cell.Fnum, Cell.Knum, corr_strength, mKL);
      /* 14-pole error */
	...
	printf("num= %4d name = %s Fnum = %3d, Knum=%3d BL/brho=% e mKl=% e\n",
      ...
      SetKLpar(lat, Cell.Fnum, Cell.Knum, mOrder=11, mKL);
...
void SetSkewQuad(LatticeType &lat)
{
  ...
  for (i = 0; i < nquad; i++)
	...
	printf("num= %4d name = %s Fnum = %3d, Knum=%3d KL=% e, KtiltL=% e\n",
	       i, Cell.PName,Cell.Fnum, Cell.Knum,M->PBpar[HOMmax+2],
	       M->PBpar[HOMmax-2]);
    }
}
			...
			double em_max, long nstepm)
{
  ...
  do
      ...
      do /* Tracking over nturn */
	{
	  ...
	  Trac(lat, x, px, tabz0[i-1L][pos], tabpz0[i-1L][pos-1L], dp2+delta,
...
#define nterm2  20
  ...
  for (i = 0; i <= Nbx; i++) {
    ...
    for (j = 0; j<= Nbz; j++) {
      ...
      Trac_Simple(lat, x, xp, z, zp, energy, 0.0, nturn, Tab, &status);
      if (status) {
	Get_NAFF(nterm2, Nbtour, Tab, fx, fz, nb_freq);
      }
      else {
  ...
  for (i = 0; i < nquad; i++)
    {
      ...
      A +=
	Cell.PL*2.0*(M->PBpar[2L + HOMmax]*Cell.Eta[0])*
	(M->PBpar[2L + HOMmax]*Cell.Eta[0]);
      i++;
    }
  fprintf(stdout,"A= %g\n", A*1.706);
}
...
****************************************************************************/
void Phase2(LatticeType &lat, long pos, double x,double px,double y, double py,
	    double energy, double ctau, long Nbtour)
{
  FILE *outf;
