...
// Read a keyword's remaining tokens as a list of element-family names,
// replacing fams. An empty list leaves the defaults in place.
static void get_fams(char *line, char **p, std::vector<std::string> &fams)
{
  std::vector<std::string> read;

  strtok_r(line, " \r", p);
  for (char *s = strtok_r(NULL, " \r", p); s != NULL;
       s = strtok_r(NULL, " \r", p))
    read.push_back(s);
  if (!read.empty())
    fams = read;
