...
// full-machine study", not "correction". What lives here is only what a corrector
// under correction/ actually consumes:
//
//   orbit_cfg   — the closed-orbit knobs cod_corr needs. Input, projected out
//                 of the param.dat knobs by orbit_config().
...
// Max no of quad corrector families listable on param.dat's ID_quads line. It
// sizes the param.dat knob (Q_Fam) and id_corr::b2 alike, so it lives here — a
// header both include — rather than in either owner. Global scope, not
// namespaced, to match the sibling limits in id_corr.h.
