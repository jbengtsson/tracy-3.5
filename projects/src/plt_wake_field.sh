...
if (ps) set output "plt_wake_field_1.".(ext)
...
set y2label "{/Symbol e}_{y} [nm{/Symbol \327}rad]"
set ytics nomirror
set y2tics
plot file_name using 1:(1e9*\$2) title "{/Symbol e}_x" with points ls 1, \
     file_name using 1:(1e9*\$4) axis x1y2 title "{/Symbol e}_y" with points \
     ls 3
...
set ytics nomirror
set y2tics
plot file_name using 1:(1e3*\$9) title "{/Symbol s}_s" with points ls 1, \
     file_name using 1:(1e3*\$8) axis x1y2 title "{/Symbol d}" with points ls 3
