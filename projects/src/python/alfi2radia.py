...
def rd_kick_map(file_name):
    ...
    with open(file_name, 'r') as f:
        for l in f:
            ...
            for k in range(3):
                vals[k] = float(vals[k])
            # [cm] -> [m]. [Gauss^2] -> [Tesla^2].
            vals[0] *= 1e-2; vals[1] *= 1e-2; vals[2] *= 1e-8
            theta.append(vals)
            # printf('%6.3f %6.3f %16.8e\n', vals[0], vals[1], vals[2])
    return np.array(theta)

def prt_RADIA(outf, L, n_x, n_y, theta_x, theta_y):
    fprintf(outf, '# Author: J.Bengtsson.\n');
...
file_name1 = 'dip_int_By_KickMap'
file_name2 = 'dip_int_Bx_KickMap'
...
prt_RADIA(outf, 0.933, 14, 11, theta_x, theta_y)
