	      ...
	      const double ksi_x, const double ksi_y, const double db3L)
{
  ...
  for (k = 1; k <= n_b3; k++) {
    ...
    if (k == 1) {
      for (j = 0; j < 2; j++)
	lat_constr.ksi1[j] = globval.Chrom[j];
    }
...
void tweak_sp(param_type &prms, constr_type &constr)
{
  ...
  if (!true)
    // 4 TBA, 1 MS, 1 SS, 1 LS: phi = 30.
    lat_constr.ini_constr(true, 1e4, 0.190, 1e0, 30.0,
			  1e-5, 1e-5, 10.0);
  else
    // 8 TBA, 4 MS, 3 SS, 1 LS: phi = 30.
    // Two Standard Straights: phi = 60.
