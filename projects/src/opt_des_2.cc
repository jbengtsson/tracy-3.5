...
const double
  ic[n_ic][2] =
  {{0.0000004297, -0.0000001526}, {3.5774796632, 3.7912591011},
   {0.0148883522, 0.0000000000}, {0.0, 0.0}},
...
// M-H6BA-18-99pm-01.14-01.
  eps0_x                = 0.097,
  dnu[]                 = {0.0/6.0, 0.0/6.0},
  high_ord_achr_nu[][2] = {{5.0/8.0, 1.0/8.0}, {4.0/8.0, 1.0/8.0}},
  beta_inj[]            = {10.7, 6.5},
  A_max[]               = {3.5e-3, 1.5e-3},
  delta_max             = 2e-2,
...
void f_der(double *b2, double *df)
{
  ...
  lat_constr.get_dchi2(twoJ, delta_max, twoJ_delta, b2, df);
}
...
double f_match(double *b2)
{
  ...
  chi2 =
    lat_constr.get_chi2(twoJ, delta_max, twoJ_delta, lat_prms, b2, false,
			false);
...
double f_achrom(double *b2)
{
  ...
  if ((lat_constr.ring && stable) || !lat_constr.ring) {
    ...
    chi2 = lat_constr.get_chi2(twoJ, delta_max, twoJ_delta, lat_prms, b2, true,
			       false);
    ...
    if (first || (chi2 < lat_constr.chi2)) {
      ...
      if (lat_constr.n_iter % n_prt == 0) {
	// Print dchi2.
	lat_constr.get_chi2(twoJ, delta_max, twoJ_delta, lat_prms, b2, true,
			    true);
...
void set_weights_std(constr_type &constr)
{
  ...
  lat_constr.mI_scl[X_]            = 1e3*1e6;
  lat_constr.mI_scl[Y_]            = 1e3*1e6;
  lat_constr.high_ord_achr_scl[X_] = 1e-10*1e6;
  lat_constr.high_ord_achr_scl[Y_] = 1e-10*1e6;
...
void set_constr_sp(constr_type &constr)
{
  ...
  for (k = 1; k <= 2; k++)
    constr.add_constr(Elem_GetPos(ElemIndex("ms"), k),
		      1e6, 1e6, 1e1*1e2,         1e1*1e3,         1e8,      1e8,
		      0.0, 0.0, beta_ms[X_], beta_ms[Y_], eta_ms_x, 0.0);
  for (k = 1; k <= 2; k++)
