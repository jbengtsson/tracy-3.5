...
plot home_dir."Dbn_rms_tol_+3.dw_90" \
     using (1e3*R_ref**2*$2):4 title "({/Symbol D}b_3L)" \
     with points ls 1, \
     home_dir."Dbn_rms_tol_-3.dw_90" \
     using (1e3*R_ref**2*$3):4 title "({/Symbol D}a_3L)" with points ls 2;
...
plot home_dir."Dbn_rms_tol_+4.dw_90" \
     using (1e3*R_ref**3*$2):4 title "({/Symbol D}b_4L)" \
     with points ls 1, \
     home_dir."Dbn_rms_tol_-4.dw_90" \
     using (1e3*R_ref**3*$3):4 title "({/Symbol D}a_4L)" with points ls 2;
...
plot home_dir."Dbn_rms_tol_+3.ivu_20" \
     ...
     using (1e3*R_ref**2*$3):4 title "({/Symbol D}a_3L)" \
     with points ls 2;
...
plot home_dir."Dbn_rms_tol_+4.ivu_20" \
     ...
     using (1e3*R_ref**3*$3):4 title "({/Symbol D}a_4L)" \
     with points ls 2;
...
set size 1.0, 0.5; set origin 0.0, 0.5;
set title "DA vs. Sys Error in Long Straights (12)";
set xlabel "{/Symbol q}_x [mrad]"; set ylabel "DA [mm^2]";
set yrange [0:];
set logscale x;
