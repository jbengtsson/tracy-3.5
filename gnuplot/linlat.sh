...
set size square;
#set xrange [0:*]
plot file_name using (1e3*\$15):(1e3*\$16):(abs(\$4)) notitle \
...
plot file_name using 3:4 axis x1y2 notitle with fsteps lt 1 lw 1 \
     lc rgb "black", \
     file_name using 3:(sqrt((\$15**2+\$16**2)*\$6)) notitle \
     with lines ls 1;
if (!ps) pause mouse "click on graph to cont.\n";
...
plot file_name using 3:4 axis x1y2 notitle with fsteps lt 1 lw 1 \
     lc rgb "black", \
     file_name using 3:(\$15**2+\$16**2):(abs(\$4)) notitle with \
     lines lt palette z;
if (!ps) pause mouse "click on graph to cont.\n";
